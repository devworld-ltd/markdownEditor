...
`sips`(macOS 전용)·`rsvg-convert`(시스템 설치)·`sharp`(네이티브 의존성) 중 무엇을 골라도 이식성이나 의존성 성질을 잃는다. 이 저장소의 **첫 화면** 런타임 의존성은 `marked`·`DOMPurify` 둘뿐이고(`mermaid` 는 F-74 에서 더했지만 다이어그램이 있을 때만 내려받는다), 아이콘 하나 때문에 그 성질을 포기할 이유가 없다.
...
이미지도 같은 R2 버킷에 저장된다. 키 형식이 달라(`<16진수16>.<확장자>`) 공유 문서(`<16진수16>`)와 섞이지 않는다.

`wrangler.jsonc` 의 `run_worker_first` 에 `/i/*` 를 넣어야 한다 — 없으면 이미지 GET 이 Worker 에 닿지 않는다(F-60 과 같은 함정).
