...
기능이 빠르게 늘어(다크 모드·단축키 안내·검색·치환·분할 조절·모드 토글·내보내기·인쇄) 한 번 훑었다. **자동 점검과 수동 시나리오 두 층**으로 본다.

#### 자동 점검만 두면 "위반 0" 이 "쓸 수 있다" 로 오해된다

이 저장소의 탭이 그 증거다 — `<div>` 에 클릭 핸들러만 달려 있어 **키보드로는 탭을 아예 바꿀 수 없었는데**, axe 는 통과했다. 그래서 키보드 도달·포커스 가시성·상태 전달을 별도 시나리오로 확인한다.

#### 고친 것

| 문제 | 증상 | 조치 |
|------|------|------|
| 탭이 `<div>` | **키보드로 탭 전환 불가** | 탭 이름을 실제 `<button>` 으로. 포커스·Enter·Space 가 브라우저에서 따라온다 |
| 프리뷰에 `tabindex` 없음 | **마우스 없이 렌더 결과를 읽어 내려갈 수 없음** | `tabindex="0"` + `role="region"` |
| `#preview` 에 role 없이 `aria-label` | `aria-prohibited-attr` (serious) | `role="region"` |
| 랜드마크 부족 | 탭바·검색 바가 어느 영역에도 속하지 않음 | `<nav aria-label>`, `role="search"` |
| 툴바 버튼 20개·탭에 포커스 스타일 없음 | UA 기본 링이 어두운 툴바에서 거의 안 보임 | `:focus-visible` 명시 |
| 활성 탭이 색으로만 구분 | 스크린리더에 전달 안 됨 | `aria-current="true"` |
| 닫기 버튼이 `×` 만 | 어느 탭을 닫는지 알 수 없음 | `aria-label="<파일명> 탭 닫기"` |
| **프리뷰 링크 색 미지정** | 다크에서 UA 기본 `#0000ee` → 대비 **1.37:1** | `--link` 토큰 (라이트 5.3:1 / 다크 6.3:1) |

마지막 항목은 F-57 이 표면과 본문은 토큰화했지만 **링크를 빠뜨린** 것이다. 자동 점검이 아니었으면 다크 모드 사용자만 겪고 보고되지 않았을 종류다.

#### 점검 도구는 런타임에 들어가지 않는다

`@axe-core/playwright` 는 `devDependencies` 다. 런타임 의존성은 여전히 `marked`·`DOMPurify` **둘뿐**이고, 빌드 산출물에 axe 는 한 바이트도 없다.
