...
| F-89 앱 중복 실행 방지 — 실제 창 포커스 전환 (AC-20, #187) | 헤드리스 Chromium 에는 macOS Launch Services·창 관리자가 없다(트랩 #82) | PWA 설치 후 실기기 수동 확인 — 결과는 이슈 #187 댓글에 기록 |
...
- [ ] (#187) PWA 설치 후 앱 창을 띄운 채 Finder 에서 `.md` 다시 열기 → **새 창이 뜨지 않고** 기존 창이 앞으로 나오며 탭이 추가된다(AC-20)
- [ ] (#187) 다른 편집기로 열린 파일을 고친 뒤 앱 창으로 돌아오기 → 깨끗한 탭은 자동 반영, 편집 중인 탭은 배너로만 알림
