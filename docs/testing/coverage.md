...
| 구성 요소 | 단위 | E2E | 판정 |
|-----------|------|-----|------|
| 웹 프론트엔드 (`src/*.ts`) | **24.19%** (라인 25.09%) | 24/27 기능 커버 | ⚠️ 개선 중 · E2E 양호 |
| 네이티브 셸 (Swift) | — | — | 🗑 제거됨 |
| 백엔드 / 어드민 | 해당 없음 | 해당 없음 | — |

전환 전 0.91% → 8.37% → 현재 **24.19%**. 이번 상승은 `swUpdate.ts`(78.9%) 한 모듈에서 나왔다. 우연이 아니라 **의존성 주입형 리프로 설계했기 때문**이다 — `SwUpdateHost` 로 능력을 주입받아 실제 `navigator.serviceWorker` 없이 검증된다. 같은 방식이 `tabs.ts`·`fileOps.ts` 에 적용되면 60% 목표는 어렵지 않다. 반대로 **DOM 결합 모듈은 여전히 0%** 다.

## 1. 단위 테스트 커버리지

```
Statements : 24.19% ( 136/562 )
Branches   : 18.50% (  47/254 )
Functions  : 23.47% (  27/115 )
Lines      : 25.09% ( 127/506 )
```
