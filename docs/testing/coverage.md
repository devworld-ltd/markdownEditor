...
| 구성 요소 | 단위 | E2E | 판정 |
|-----------|------|-----|------|
| 웹 프론트엔드 (`src/*.ts`) | **76.68%** (라인 77.33%) | 35/38 기능 커버 | ✅ 목표 초과 |
| 네이티브 셸 (Swift) | — | — | 🗑 제거됨 |
| 백엔드 / 어드민 | 해당 없음 | 해당 없음 | — |
...
| `search.ts` | 73.04% | 77.5% | 80.95% | **73.83%** | 미커버는 미러 측정(`measureOffsetTop` 기본 구현) — 실제 레이아웃이 필요해 E2E 가 맡는다 |
