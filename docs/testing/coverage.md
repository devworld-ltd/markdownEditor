...
| 구성 요소 | 단위 | E2E | 판정 |
|-----------|------|-----|------|
| 웹 프론트엔드 (`src/*.ts`) | **8.37%** (라인 8.41%) | 23/26 기능 커버 | ⚠️ 단위 취약 · E2E 양호 |
| 네이티브 셸 (Swift) | — | — | 🗑 제거됨 |
| 백엔드 / 어드민 | 해당 없음 | 해당 없음 | — |

전환 전 0.91% → 현재 8.37%. 개선폭은 `storage.ts` 테스트에서 나왔고, **DOM 결합 모듈은 여전히 0%** 다. `notice.ts`·`sw.js` 등 신규 코드도 단위 미검증이라 분모만 늘었다.

## 1. 단위 테스트 커버리지

```
Statements : 8.37% ( 36/430 )
Branches   : 7.21% ( 14/194 )
Functions  : 9.19% (  8/87 )
Lines      : 8.41% ( 33/392 )
```
...
| `storage.ts` | **94.28%** | 93.33% | **100%** | 96.77% | 용량 초과 테스트 추가로 상승. 미커버는 storage 접근 자체가 throw 하는 경로 |
| `preview.ts` | 0% | 100% | 0% | 0% | 분기 없음 |
| `editor.ts` | 0% | 0% | 0% | 0% | 디바운스 미검증 |
| `tabs.ts` | 0% | 0% | 0% | 0% | **상태 로직 전량 미검증 (323줄)** |
...
| `public/sw.js` | 캐시 전략 3분기, 구버전 캐시 정리 | Service Worker 테스트 하니스 또는 오프라인 E2E |
