...
| F-87 | 좁은 화면 키보드 위 서식 바 (#155) | `src/formatBar.ts` | 단위 7 + E2E 8 |
...
| F-47 | 단위 테스트 커버리지 | ✅ **81% 유지**(목표 60%) — [커버리지 분석](../testing/coverage.md). 남은 0%: `toolbar.ts`·`main.ts`(E2E 담당) · `caretStatus.ts`·`formatBar.ts`(#152·#155, 주입형 리프인데 단위 없음) |
...
> 예전에 여기 있던 셋(F-68 아이콘 · F-58 단축키 안내 · F-25/F-22)은 **전부 완료됐다.**
> 닫힌 항목을 남겨 두면 다음 사람이 이미 있는 기능을 다시 만든다.

1. **F-84 / F-87 단위 커버리지** — `caretStatus.ts`·`formatBar.ts` 가 주입형 리프인데 단위 테스트가 없다(0%). E2E 로만 덮여 있다.
2. **F-87 실기기 확인** — 가상 키보드 위 서식 바는 헤드리스로 검증할 수 없다. 절차는 이슈 #155 댓글.
