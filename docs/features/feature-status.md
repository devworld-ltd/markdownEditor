...
| F-69 | 서비스 워커 업데이트 알림 UI | 새 버전 배포 시 "새로고침하세요" 안내 없음 |
...
1. **F-47** — 단위 커버리지 8%대. DOM 결합 모듈이 전부 미검증이라 리팩터링 안전망이 없다.
2. **F-69** — 서비스 워커 도입으로 "새 배포가 있는데 사용자는 옛 버전을 보는" 구간이 생겼다. 안내 UI 가 필요하다.
3. **F-64** — 배포 후 자동 헬스체크가 없어 배포 실패를 수동으로 확인해야 한다.
