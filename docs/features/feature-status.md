...
| F-86 | 좁은 화면 편집/미리보기 세그먼트 (#154) | `src/viewMode.ts` | 단위 7 + E2E 10 |
| F-87 | 좁은 화면 키보드 위 서식 바 (#155) | `src/formatBar.ts` | 단위 7 + E2E 8 + **실기기 확인 2026-08-18** (iOS Safari·Android Chrome) |
| F-18 (수정) | 미리보기 코드 블록 들여쓰기 버그 (#180) | `src/style.css` | E2E 6 |
...
| F-90 | 원격 URL 열기 — `?url=<https 주소>` 로 진입하면 확인 대화상자 뒤 fetch·정화(F-18)·새 탭. CORS/네트워크/HTTP/용량 4갈래 오류 구별, `connect-src https:` 완화 (#195). `http:` 예외는 **앱 자신이 로컬일 때만** (#198) | `src/openParams.ts`, `src/remoteDoc.ts`, `src/openUrlUi.ts` | 단위 59(`openParams`·`remoteDoc`·`openUrlUi`) + E2E 24(`openUrl.spec.ts`, CORS 갈래는 단위로 하향) |
...
| F-47 | 단위 테스트 커버리지 | ✅ **84% 유지**(목표 60%) — [커버리지 분석](../testing/coverage.md). 남은 0%: `main.ts`(E2E 담당). `toolbar.ts` 15% |
...
> 예전에 여기 있던 넷(F-68 아이콘 · F-58 단축키 안내 · F-25/F-22 · F-87 실기기 확인)은
> **전부 완료됐다.** 닫힌 항목을 남겨 두면 다음 사람이 이미 있는 기능을 다시 만든다.

**현재 즉시 조치가 필요한 항목은 없다.**
