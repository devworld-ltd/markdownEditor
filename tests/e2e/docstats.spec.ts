...
test("DS1: 기본으로 통계가 보인다", async ({ page }) => {
  ...
  await expect(page.locator("#status-bar")).toHaveText("빈 문서");
});

test("DS2: 입력하면 통계가 갱신된다", async ({ page }) => {
  await page.locator("#editor").fill("한글 문서 입니다");
  await expect(page.locator("#status-bar")).toContainText("3어절");
  await expect(page.locator("#status-bar")).toContainText("9자"); // 공백 2개 포함
});

test("DS3: 영문 문서는 '단어' 로 표시한다", async ({ page }) => {
  await page.locator("#editor").fill("one two three");
  await expect(page.locator("#status-bar")).toContainText("3단어");
});
...
test("DS4: 선택하면 그 범위의 통계를 보여준다", async ({ page }) => {
  ...
  await expect(page.locator("#status-bar")).toContainText("선택");
  await expect(page.locator("#status-bar")).toContainText("2어절");
});
...
test("DS6: 인쇄에는 나오지 않는다", async ({ page }) => {
  ...
  const visible = await page.evaluate(
    () => document.querySelector("#status-bar")!.getClientRects().length > 0,
  );
...
test("DS7: 큰 문서에서도 입력이 느려지지 않는다", async ({ page }) => {
  ...
  await expect(page.locator("#status-bar")).toContainText("어절");
...
test("DS8: 스크린리더가 매 글자를 읽지 않는다", async ({ page }) => {
  ...
  await expect(page.locator("#status-bar")).not.toHaveAttribute("aria-live", /.*/);
});
