...
interface ToolbarAction {
  ...
  /**
   * #147: 이 버튼이 속한 묶음. 같은 값이 연속하면 한 `role="group"` 으로 묶인다.
   *
   * **묶음은 눈으로만 나뉘는 것이 아니다** — 구분선은 스크린리더에게 아무 뜻도
   * 아니므로, 같은 값끼리 묶어 `aria-label` 을 붙여야 "파일 그룹" 으로 읽힌다.
   */
  group?: string;
  /**
...
/**
 * #147: 툴바 묶음의 **표시 순서**. 배열 선언 순서가 아니라 이것이 화면 순서다.
 *
 * 구분선을 손으로 놓지 않는 이유: 버튼을 하나 옮길 때마다 `separator: true` 를
 * 같이 옮겨야 하는데, 그 둘이 어긋나도 **화면에서만 티가 나고 타입은 통과한다.**
 * 묶음 경계에서 자동으로 그리면 어긋날 수가 없다.
 *
 * `부가` 는 #149 에서 "더보기" 팝오버로 옮겨질 묶음이다. 팝오버가 생기기 전에
 * 여기서 빼면 **코드 블록·구분선·HTML 내보내기·HTML 복사는 단축키가 없어
 * 어디로도 닿을 수 없게 된다** — 그래서 지금은 툴바에 남긴다.
 */
const GROUP_ORDER = ["파일", "강조", "블록", "삽입", "부가", "공유", "보기·도움"] as const;
...
const actions: ToolbarAction[] = [
  ...
  {
    // F-30 표. 커서가 표 안이면 정렬 편집, 밖이면 삽입으로 열린다.
    label: "Table",
    group: "삽입",
    icon: `<svg ${SVG_ATTRS}><rect x="3" y="4" width="18" height="16" rx="2"/><line x1="3" y1="10" x2="21" y2="10"/><line x1="9" y1="10" x2="9" y2="20"/><line x1="15" y1="10" x2="15" y2="20"/></svg>`,
    action: () => fileActions?.showTable?.(),
  },
  {
  ...
  {
    // #141 사용 설명서. 웹에는 메뉴바가 없어 "도움말" 이 갈 자리가 없다 —
    // 안내 성격인 단축키 버튼 옆에 둔다.
    label: "Manual",
    group: "보기·도움",
    icon: `<svg ${SVG_ATTRS}><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>`,
    action: () => fileActions?.showManual?.(),
  },
  {
...
export function initToolbar(
  ...
  ordered.forEach(({ label, icon, action, editsText, id, group }) => {
    // 묶음이 바뀌면 구분선을 하나 놓고 새 컨테이너를 연다.
    if (group !== groupName) {
      if (groupName !== null) {
        const sep = document.createElement("div");
        sep.className = "toolbar-separator";
        // 구분선은 장식이다 — 스크린리더에게는 묶음의 `aria-label` 이 그 역할을 한다.
    ...
    (groupEl ?? container).appendChild(btn);
  });
}
